...
function cargar_datos_defunciones() {
    ...
    if (result == 0) {
        var ruta_arch_adjunto = $("#fil_arch").val();
        var extension = obtener_extension_archivo(ruta_arch_adjunto);
        if (extension != "csv") {
            alert("El archivo a cargar debe ser un documento CSV ");
            return;
        }
        ...
        llamarAjaxUploadFiles("defunciones_ajax.php", params, "d_carga_defunciones", "", "", "fil_arch");
    } else {
...
function finalizar_cargar_datos(ind_resultado) {
    ...
    if (ind_resultado > 0) {
        if (ind_resultado == 1) {
            ...
            //setTimeout('$("#d_contenedor_exito").css("display", "none")', 3000);
        }
